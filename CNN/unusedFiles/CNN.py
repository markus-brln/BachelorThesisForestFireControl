...
def create_model(n_timesteps, n_features, n_outputs):
    ...
    # basic model
    model = Sequential([
        Convolution1D(input_shape=(n_timesteps, n_features), kernel_size=3, filters=64, activation='relu', kernel_regularizer='l2', name="conv1"),
...
def retrain_vs_single_fnfpacc():
    ...
    tfile.write("mean squared error, rates of false positives and false negatives per patient and accuracy for\n1) single\n2) retrain\n\n")
    tfile.write("patient  mse               fp                  fn                 acc\n")

    for single_name, retrain_name in zip(glob.glob("saved_models/cnn_patient*"), glob.glob("saved_models/cnn_retrained*")): # exclude general model
        print(single_name[-3:])
        ...
        tfile.write(str(single_name[-3:]) + " " + str(perf_single[0]) + " " + str(perf_single[1]/len(y)) + " " + str(perf_single[2]/len(y)) + " " + str(perf_single[3]) + "\n")
        tfile.write(str(retrain_name[-3:]) + " " + str(perf_retrain[0]) + " " + str(perf_retrain[1] / len(y)) + " " + str(perf_retrain[2] / len(y)) + " " + str(perf_retrain[3]) + "\n\n")
