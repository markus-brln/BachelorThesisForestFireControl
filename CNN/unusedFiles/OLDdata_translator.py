...
def raw_to_IO_arrays(data):
    ...
    for i in range(len(data)):
        ...
        # if len(agent_specific) == n_agents:       # some data points don't have the right amount of agents
        for j in range(n_agents):
            agent_positions.append([agent_specific[j][0][0] / env_dim,
                                    agent_specific[j][0][1] / env_dim])  # x, y of agent that needs waypoint
...
def construct_input_images_only(data):
    ...
    for single_image, data_point, i in zip(input_images_single, data, range(0, len(data))):
        ...
        for active_pos in agent_positions:
            agent_image = np.copy(single_image)
            agent_image[active_pos[0] - apd: active_pos[0] + apd, active_pos[1] - apd: active_pos[1] + apd,
            5] = 1  # mark position of active agent, channel [5]

            for others_pos in agent_positions:
                if others_pos != active_pos:
                    # agent_image[others_pos[0]][others_pos[1]][4] = 1  # mark position of other agents, channel [4]
                    agent_image[others_pos[0] - apd: others_pos[0] + apd, others_pos[1] - apd: others_pos[1] + apd,
                    4] = 1
...
def raw_to_IO(data, NN_variant):
    ...
    if NN_variant == 'input':
        images, concat = construct_input_bigAgents(
            data)  ## creates a box around the agent to increase weight of agent position in the model
    else:
...
if __name__ == "__main__":
    ...
    data = load_raw_data(file_filter="mEASYFIVEBASIC")  # "mXYEASYFIVE")
    data = data[:100]
