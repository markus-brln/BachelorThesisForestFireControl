...
class View:
    ...
    def show_selected(self, block_size):
        ...
        for selected in self.model.selected_squares:
            rect = pygame.Rect(selected[0] * block_size - block_size / 2, selected[1] * block_size, block_size,
                               block_size)
    ...
    def startFire(self):
        ...
        fire = pygame.Rect(x - 20, y - 20, self.model.size, self.model.size)
        pygame.draw.rect(self.window, (200, 0, 0), fire)


def main():
    environment = Model(20, [])
    view = View(environment, 20)
    running = True
    while running:
